...
def get_by_request():
    username = 'insta360'
    url = 'http://weibo.com/'+ username + '?is_all=1'
    headers = {}
    ...
    page = response.read()
    pattern = re.compile("\((.{0,10})\)<\\\\\/a><\\\\\/h2>", re.S)
...
def get_by_selenium():
    ...
    try:
        result = int(
            wait.until(lambda x: x.find_element_by_xpath('//*[@id="Pl_Core_T8CustomTriColumn__3"]/div/div/div/table/tbody/tr/td[2]/strong').text))
